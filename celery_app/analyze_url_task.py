...
def send_http_302(redirect, pk):
    ...
    resp = IP(dst=pk['ip_src'], src=pk['ip_dst']) / TCP(dport=pk['tcp_sport'], sport=pk['tcp_dport'], flags="PA", seq=pk['tcp_ack'],ack=pk['tcp_seq'] + pk['tcp_payload_len']) / Raw(load=http_payload)
    send(resp)
